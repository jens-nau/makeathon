...
def evaluate(args):
    ...
    test_dataset = Dataset(args.data_directory, args.task,
    ...
    losses = []
    log_every = 20
    for i, (image, image1, image2, label, weight, record) in enumerate(test_loader):
        ...
        prediction, attention1, attention2, attention3 = model.forward(
            image.float(), image1.float(), image2.float())
        prediction = prediction.squeeze(0)

        attention_path = './attention/sagittal/' + record[0] + '.npy'
        if os.path.exists(attention_path):
            os.remove(attention_path)
        np.save(attention_path, attention1.detach().cpu().numpy())

        attention_path = './attention/axial/' + record[0] + '.npy'
        if os.path.exists(attention_path):
            os.remove(attention_path)
        np.save(attention_path, attention2.detach().cpu().numpy())

        attention_path = './attention/coronal/' + record[0] + '.npy'
        if os.path.exists(attention_path):
            os.remove(attention_path)
        np.save(attention_path, attention3.detach().cpu().numpy())
